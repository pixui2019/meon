...
sql="select * from mobile_web_ads"
rs.open(sql),cn,1,3
rs.addnew
rs("name")=l_name
rs("url")=l_url
...
sql="select top 1 [id] from [mobile_web_ads] where [name]='"&l_name&"' order by [time] desc"
rs2.open(sql),cn,1,1
if not rs2.eof  then
l_id=rs2("id")
call ADs_to_js(l_id)
...
<%
call index_to_html()
response.Write "<script language='javascript'>alert('添加成功！');location.href='mobile_ads_list.asp';</script>"
end if 
 %>
