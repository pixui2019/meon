...
<!-- #include file="../inc/m_x_to_html/ADs_to_js.asp" -->
...
sql="select * from mobile_web_ads where id="&l_id&""
rs.open(sql),cn,1,3
rs("name")=l_name
rs("url")=l_url
...
<%
call index_to_html()
response.Write "<script language='javascript'>alert('修改成功！');location.href='mobile_ads_list.asp?page="&page&"&act="&act&"&keywords="&keywords&"';</script>"
end if 
 %>
...
%>
<% set rs=server.createobject("adodb.recordset")
sql="select * from mobile_web_ads where id="&article_id&""
rs.open(sql),cn,1,1
	 ...
	 <tr>
	    ...
	    <td class='forumRowHighLight'><label>
	      <select name="position" id="position">
		   ...
		   sql="select id,name from mobile_web_ads_position "
		   rsp.open(sql),cn,1,1
