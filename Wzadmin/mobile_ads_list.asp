...
if cid=""  then
s_sql="select * from mobile_web_ads where [name]  like '%"&keywords&"%'  order by [position]"
else
search_sql="and [position]="&cid&""
s_sql="select * from mobile_web_ads where [name] like '%"&keywords&"%'"&search_sql&"  order by [order]"
end if

else
s_sql="select * from mobile_web_ads order by [position]"
...
sql="select id,order from mobile_web_ads where id="&id1&""
rs1.open(sql),cn,1,3
...
%>
	...
	<tr><td height="400" valign="top"  class='forumRow'><br>
	    ...
	    <table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
          <tr>
            <td height="25" class='forumRowHighLight'>&nbsp;| <a href="mobile_ads_add.asp">添加新的广告</a></td>
          </tr>
          <tr>
            <td height="30"></td>
          </tr>
...
<% '文章列表模块
strFileName="mobile_ads_list.asp" 
pageno=20
...
class_style="forumRowHighLight"
          ...
          <tr >
           ...
           <td class='<%=class_style%>' ><%=rs("name")%></td>
			...
			sql="select name from mobile_web_ads_position where [id]="&rs("position")&""
			rst.open(sql),cn,1,1
            ...
            <td class='<%=class_style%>' ><div align="center"><a href="mobile_ads_view_yes.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>"><%if rs("view_yes")=1 then%>已审核<%else%><span style="color: #FF0000">未审核</span><% end if%></a></div></td>
            <td class='<%=class_style%>' ><div align="center"><%=rs("time")%></div></td>
            <td class='<%=class_style%>' >
            <div align="center"><a href="mobile_ads_edit.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>">修改</a> | <a href="javascript:if(ask('警告：删除后将不可恢复，可能造成意想不到后果？')) location.href='mobile_ads_del.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>';">删除</a>            </div></td>
          </tr></form>
		  		  <%
		  rs.movenext
		  next
...
sqlClass1="select id,name from mobile_web_ads_position  order by id" 
