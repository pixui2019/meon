...
%>
	...
	<tr><td height="400" valign="top"  class='forumRow'><br>
	    <table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
          <tr>
            <td height="25" class='forumRowHighLight'>&nbsp;| <a href="mobile_ads_position_add.asp">添加新的广告位置</a></td>
          </tr>
          <tr>
            <td height="30"></td>
          </tr>
...
<% '文章列表模块
strFileName="mobile_ads_position_list.asp" 
pageno=25
set rs = server.CreateObject("adodb.recordset")
s_sql="select * from mobile_web_ads_position order by  time asc"
...
class_style="forumRowHighLight"
          ...
          <tr >
           ...
           <td class='<%=class_style%>' ><div align="center"><%=rs("name")%></div></td>
            ...
            <div align="center"><a href="mobile_ads_position_edit.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>">修改</a> | <a href="javascript:if(ask('警告：删除后将不可恢复，可能造成意想不到后果？')) location.href='mobile_ads_position_del.asp?id=<%=rs("id")%>&page=<%=page%>&act=<%=act%>&keywords=<%=keywords%>';">删除</a>            </div></td>
          </tr></form>
		  		  <%
		  rs.movenext
		  next
