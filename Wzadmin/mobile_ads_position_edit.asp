...
sql="select * from mobile_web_ads_position where id="&l_id&""
rs.open(sql),cn,1,3
rs("name")=l_name
rs("memo")=l_memo
rs.update
rs.close
set rs=nothing
response.Write "<script language='javascript'>alert('修改成功！');location.href='mobile_ads_position_list.asp?page="&page&"&act="&act&"&keywords="&keywords&"';</script>"
end if 
 %>
...
%>
<% set rs=server.createobject("adodb.recordset")
sql="select * from mobile_web_ads_position where id="&article_id&""
rs.open(sql),cn,1,1
