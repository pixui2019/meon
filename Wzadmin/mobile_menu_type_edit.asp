...
sql="select * from mobile_web_menu_type where id="&l_id&""
rs.open(sql),cn,1,3
rs("name")=l_name
rs("Enname")=l_Enname
rs("memo")=l_memo
...
set rs=nothing
call index_to_html()
response.Write "<script language='javascript'>alert('修改成功！');location.href='mobile_menu_type_list.asp?page="&page&"&act="&act&"&keywords="&keywords&"';</script>"
end if 
 %>
...
%>
<% set rs=server.createobject("adodb.recordset")
sql="select * from mobile_web_menu_type where id="&article_id&""
rs.open(sql),cn,1,1
