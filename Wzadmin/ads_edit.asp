...
l_ClassID=trim(request.form("ClassID"))
...
<%
call index_to_html()
response.Write "<script language='javascript'>alert('OK');location.href='ads_list.asp?page="&page&"&act="&act&"&keywords="&keywords&"';</script>"
end if 
 %>
...
return true;}
	...
	<td class='forumRowHighLight'><input name='name' type='text' id='name'  value="<%=rs("name")%>"size='70'>
	 ...
	 <tr>
	    <td class='forumRow' height=23>选择栏目 (选填) </td>
	    <td class='forumRow'><label>
	      <select name="ClassID" id="ClassID">
		  <option value="0">请选择</option>
	       <% set rsp=server.createobject("adodb.recordset")
		   sql="select id,name from [category] where ppid=1 "
		   rsp.open(sql),cn,1,1
		   if not rsp.eof then
		   do while not rsp.eof 
		   %> <option value="<%=rsp("id")%>" <%if rsp("id")=rs("ClassID") then
		response.write "selected"
		end if%>><%=rsp("name")%></option>
            <%
			rsp.movenext
