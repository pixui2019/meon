...
'文章列表模板类型获取
set rs_1=server.createobject("adodb.recordset")
sql="select FileName,FolderName from mobile_web_Models_type where [id]=49"
rs_1.open(sql),cn,1,1
...
Block_LeftClassList=Block_LeftClassList&"<li class='current'><A href='/"&web_Folder&"/"&ClassFolder1&"/"&rsl("Folder")&"'>"&rsl("name")&"</A> "
else
...
do while not rs.eof 
Block_LeftClassList=Block_LeftClassList&"<li><a href='/"&web_Folder&"/"&ClassFolder1&"/"&rsl("Folder")&"/"&rs("folder")&"/' >"&rs("name")&"</a></li> "
rs.movenext
loop
Block_LeftClassList=Block_LeftClassList&"</ul>"
end if
...
<%
 '文章列表读取替换
sql="select [id] from [article] where "&ClassSQL&"='"&ClassID&"' and view_yes=1  and ArticleType=3 order by [OrderCount],[time] desc"
set rs1=server.createObject("ADODB.Recordset")
rs1.open sql,cn,1,1
%><%
if not rs1.eof then
...
sql="select [title],[image],[url],[file_path],[image],description from [article] where "&ClassSQL&"='"&ClassID&"' and view_yes=1  and ArticleType=3 order by [OrderCount],[time] desc"
set rs=server.createObject("ADODB.Recordset")
rs.open sql,cn,1,1
if not rs.eof then
...
list_block=list_block&"<div class='NewsLi'><div class='NewsLiLeft'> <ul>"
list_block=list_block&"<li><a href='"&rs_url&"' target='_blank'><img src='/images/up_images/"&RSImage&"'  alt='"&rs("title")&"'></a></p></div>"
list_block=list_block&"<div class='NewsLiRight'>"
list_block=list_block&"<div class='NewsLiTitle'><a href='"&rs_url&"' target='_blank' title='"&rs("title")&"'>"&left(rs("title"),29)&"</a></div>"
list_block=list_block&"<p>"&left(ClearHtml(rs("description")),30)&"...</p></div>"
list_block=list_block&"<div class='clearfix'></div></div>"
