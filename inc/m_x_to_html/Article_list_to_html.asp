...
'文章列表模板类型获取
set rs_1=server.createobject("adodb.recordset")
sql="select FileName,FolderName from mobile_web_Models_type where [id]=4"
rs_1.open(sql),cn,1,1
...
sql="select [id],[title],[content],[url],[description],[author],[cid],[file_path],[time],[image],[hit] from [article] where "&ClassSQL&"='"&ClassID&"' and view_yes=1  and ArticleType=1 order by [time] desc"
set rs=server.createObject("ADODB.Recordset")
rs.open sql,cn,1,1
if not rs.eof then
