...
'顶部广告读取
set rs=server.createobject("adodb.recordset")
sql="select top 10 [id],ADtype,[ADcode],[image],[url] from web_ads  where [position]=35 and view_yes=1 and ClassID="&ClassID1&" order by [time] desc"
rs.open(sql),cn,1,1
if not rs.eof then
for i=1 to rs.recordcount
if rs("adcode")<>"" then
...
sql="select [id],[title],[content],[url],[keywords],[description],[cid],[file_path],[time],[Hit] from [article] where "&ClassSQL&"='"&ClassID&"' and view_yes=1  and ArticleType=1 order by [time] desc"
set rs=server.createObject("ADODB.Recordset")
rs.open sql,cn,1,1
if not rs.eof then
...
list_block=list_block&"<li><div class='time'><strong>"&day(rs("time"))&"</strong>"&year(rs("time"))&"-"&month(rs("time"))&"</div>"
list_block=list_block&"<div class='time-txt'>"
list_block=list_block&"<h3><a href='"&rs_url&"' target='_blank'>"&left(rs("title"),35)&"</a></h3>"
list_block=list_block&"<p>"&left(ClearHtml(rs("description")),120)&"...</p>"
list_block=list_block&"</div><div class='clearfix'></div></li>"
