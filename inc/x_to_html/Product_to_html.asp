...
set rs=nothing
set rs=server.createobject("adodb.recordset")
sql="select top 6 title,content,file_path,[url],image from [article]  where  cid='"&NewsID&"'  and view_yes=1  and ArticleType=2 and index_push=1 order by [time] desc"
rs.open(sql),cn,1,1
if not rs.eof and not rs.bof then
...
Block01_LeftItem=Block01_LeftItem&"<dd><a href='"&rs_url&"' target='_blank' title='"&rs("title")&"'><img src='/images/up_images/"&rs("image")&"' width='90' height='90' alt='"&rs("title")&"'> "&left(rs("title"),14)&"</a></dd>"
rs_0.close
set rs_0=nothing
rs.movenext
next
...
MorePro_List=MorePro_List&"<div class='albumblock'><div class='inner'><a href='"&rs_url&"' target='_blank'  title='"&left(rsp("title"),39)&"'><img src='/images/up_images/"&rsp("image")&"' alt='"&left(rsp("title"),39)&"'/></a><div class='albumtitle'><a href='"&rs_url&"' target='_blank'  title='"&left(rsp("title"),39)&"'>"&left(rsp("title"),15)&"</a></div></div></div>"
rsp.movenext
loop
else
...
sql="select name from [category] where [id]="&rs("pid")&" "
rso.open(sql),cn,1,1
article_class=rso("name")
rso.close
set rso=nothing
end if
...
'if i=0 then
'Article_Pics=Article_Pics&"<li><a href=""#"" class=""on""><img src=""/images/up_images/"&PicsContent(i)&""" /></a></li> "
'else
Article_Pics=Article_Pics&"<li><img src=""/images/up_images/"&PicsContent(i)&""" /></li> "
'end if
...
'顶部广告读取
set rs=server.createobject("adodb.recordset")
sql="select top 10 [id],ADtype,[ADcode],[image],[url] from web_ads  where [position]=35 and view_yes=1 and ClassID="&ClassID&" order by [time] desc"
rs.open(sql),cn,1,1
if not rs.eof then
for i=1 to rs.recordcount
if rs("adcode")<>"" then
...
web_TopMenu=""
set rs=server.createobject("adodb.recordset")
sql="select name,id,url,enname from web_menu_type where TopNav=1 order by [order]"
rs.open(sql),cn,1,1
if not rs.eof then
web_TopMenu=web_TopMenu&"<ul id='sddm'>"
for i=1 to rs.recordcount
if rs("url")="/"&ClassFolder1&"/" then
...
sql="select url,name from web_menu where view_yes=1 and [position]="&rs("id")&" order by [order]"
rss.open(sql),cn,1,1
if not rss.eof then
web_TopMenu=web_TopMenu&"<li class='"&CurrentClass&"'><a href='"&rs("url")&"' onmouseover=mopen('m"&i&"') onmouseout='mclosetime()'>"&rs("name")&"</a> "
...
web_TopMenu=web_TopMenu&"<li  class='"&CurrentClass&"'><a href='"&rs("url")&"'>"&rs("name")&"</a></li> "
end if
rss.close
set rss=nothing
...
replace_code=replace(replace_code,"$article_class$",article_class)
...
replace_code=replace(replace_code,"$Block03_LeftHotItem$",Block03_LeftHotItem)
