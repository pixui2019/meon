...
if rs("url")=Model_FolderName&"/" then
...
'顶部广告读取
set rs=server.createobject("adodb.recordset")
sql="select top 10 [id],ADtype,[ADcode],[image],[url] from web_ads  where [position]=35 and ClassID=0 and view_yes=1 order by [time] desc"
rs.open(sql),cn,1,1
if not rs.eof then
for i=1 to rs.recordcount
if rs("adcode")<>"" then
