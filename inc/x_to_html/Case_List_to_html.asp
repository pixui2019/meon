...
<%
 '文章列表读取替换
sql="select * from [article] where "&ClassSQL&"='"&ClassID&"' and view_yes=1  and ArticleType=2 order by OrderCount,[time] desc"
set rs1=server.createObject("ADODB.Recordset")
rs1.open sql,cn,1,1
%><%
if not rs1.eof then
...
list_block=list_block&"<div class='albumblock'><div class='inner'><a href='"&rs_url&"' target='_blank'  title='"&left(rs("title"),39)&"'><img src='/images/up_images/"&rs("image")&"' title='"&left(rs("title"),39)&"'/></a><div class='albumtitle'><a href='"&rs_url&"' target='_blank'  title='"&left(rs("title"),39)&"'>"&left(rs("title"),30)&"</a></div></div></div>"
